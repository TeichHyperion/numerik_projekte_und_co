...
subroutine converge_derivative(grid_inout, function_values_inout, number_grid_points, derivative_in, absolut_tol, relative_tol, derivative_out, grid_type_sub)
    ...
    do while (.not. converged) 
        ...
        if (j >= 22 .or. size(grid_inout) > 10**6) then
            print *,  "Exited without convergence at iteration ", j, "and gridsize", size(grid_inout)
            exit 
        end if
...
subroutine central_difs(y, grid_in, y_prime)
    ...
    real(dp), allocatable                :: y_co(:)[:] 
    real(dp), allocatable, intent(out)   :: y_prime(:)
    real(dp), allocatable                :: y_prime_co(:)[:]
    real(dp)                             :: h
    real(dp), intent(in)                 :: grid_in(:)
    real(dp), allocatable                :: grid_co(:)[:]
    ...
    allocate(y_co(size(y))[*]   )

    !dx = grid_in(2:) - grid_in(1:size(grid_in)-1)
    call co_broadcast(y_co, sender)

    print*, this_image(), y_co(1)[1]
...
subroutine central_difs_4th_uniform(y, grid_in, y_prime)
    ...
    ! third order backward diffs
    y_prime(n_elements)   = (25 * y(n_elements)      + & ! everything multiplied by 12 to avoid division and reals
                            -48 * y(n_elements-1)    + &
                             36  * y(n_elements-2)   + &
                            -16  * y(n_elements-3)   + &
                             3  * y(n_elements-4)  ) / &
                             (12d0* h)
    y_prime(n_elements-1) = (25 * y(n_elements-1)    + &
                            -48 * y(n_elements-2)    + &
                             36  * y(n_elements-3)    + &
                            -16  * y(n_elements-4)  + &
                            3  * y(n_elements-5)    )/ &
    ...
    y_prime(1)   = (-25 * y(1)      + & ! everything multiplied by 12 to avoid division and reals
                    +48 * y(1+1)    + &
                    -36  * y(1+2)    + &
                    +16  * y(1+3)  + &
                    -3  * y(1+4))  / &
                    (12d0* h)
    y_prime(2)   = (-25 * y(2)      + & ! everything multiplied by 12 to avoid division and reals
                    +48 * y(2+1)    + &
                    -36  * y(2+2)    + &
                    +16  * y(2+3)  + &
                    -3  * y(2+4)  ) / &
